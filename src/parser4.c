...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/24 02:45:56 by pzaw              #+#    #+#             */
/*   Updated: 2025/04/24 02:45:57 by pzaw             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_list	*get_redir(t_list **tokens)
{
	...
	tmp_list = *tokens;
	while (tmp_list != NULL
		&& is_redirection_token(((t_token *)tmp_list->content)->type)
...
char	*get_argument(t_list **tmp_list, char *tmp, t_list *tokens)
{
	...
	if (token_content(*tmp_list)->space == 0
		&& (*tmp_list)->next != NULL && *tmp_list != tokens)
		arg = join_tokens(tmp_list, ft_strdup(token_content(*tmp_list)->token));
	else if (ft_strcmp(token_content(*tmp_list)->token, tmp) == 0
		&& (*tmp_list)->next != NULL)
		arg = join_tokens(tmp_list, ft_strdup(token_content(*tmp_list)->token));
	else
