...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/24 02:51:42 by pzaw              #+#    #+#             */
/*   Updated: 2025/04/24 02:51:44 by pzaw             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	validate_redirection(t_list *tmp_list)
{
	...
	result = (tmp_list != NULL
			&& is_word_token(((t_token *)tmp_list->content)->type)
	...
	t_token *current_token, t_list *tmp_list)
{
	...
	new_redir = create_redir(((t_token *)tmp_list->content)->token,
			current_token->type);
	if (!new_redir)
	{
