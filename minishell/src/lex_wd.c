...
int	lex_token_variable(char *str, int *i, t_list **l_token)
{
	...
	if (v_len > 0)
	{
		if (space)
        {
            // Include one space after the variable
            str_token = malloc(v_len + 2); // +1 for space, +1 for null terminator
            if (str_token == NULL)
            {
                print_error(__func__, __FILE__, __LINE__, "Mem allocate failed for str_token");
                return (1);
            }
            ft_strlcpy(str_token, &str[*i], v_len + 1); // Copy the variable
            str_token[v_len] = '*'; // Add the space
            str_token[v_len + 1] = '\0'; // Null-terminate
        }
        else
        {
            // No space after the variable
